...
**Read Also**: [Manage Linux Files Effectively using commands head, tail and cat](https://www.tecmint.com/view-contents-of-file-in-linux/)
...
It’s difficult to search particular string due to length of dmesg output. So, filter the lines with are having string like ‘**usb**‘ ‘**dma**‘ ‘**tty**‘ and ‘**memory**‘ etc. The ‘-i’ option instruct to [grep command](https://www.tecmint.com/12-practical-examples-of-linux-grep-command/) to ignore the case (upper or lower case letters).
